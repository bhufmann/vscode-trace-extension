...
export class TraceViewerPanel {
    ...
    private doHandleVscodeWebViewReady(): void {
        ...
        if (this._experiment) {
            ...
            const data = { data: getTspClientUrl(), experiment: wrapper };
            this._messenger.sendNotification(setTspClient, this._webviewParticipant, data);
        } else {
            this._messenger.sendNotification(setTspClient, this._webviewParticipant, getTspClientUrl());
        }
        this.loadTheme();
    }
